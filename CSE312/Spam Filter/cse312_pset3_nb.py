...
class NaiveBayes():
    ...
    def fit(self, train_hams:List[str], train_spams:List[str]):
        ...
        total_train_emails = self.num_train_hams + self.num_train_spams
        self.spam_before = self.num_train_spams / total_train_emails
        self.ham_before = self.num_train_hams / total_train_emails
    ...
    def predict(self, filename:str) -> str:
        ...
        log_prob_spam = np.log(self.spam_before)
        log_prob_ham = np.log(self.ham_before)

        for word in words:
            if word in self.word_counts_spam:
                log_prob_word_spam = np.log((self.word_counts_spam[word] + 1) / (self.num_train_spams + 2))
            else:
                log_prob_word_spam = np.log(1 / (self.num_train_spams + 2))

            if word in self.word_counts_ham:
                log_prob_word_ham = np.log((self.word_counts_ham[word] + 1) / (self.num_train_hams + 2))
            else:
                log_prob_word_ham = np.log(1 / (self.num_train_hams + 2))

            log_prob_spam += log_prob_word_spam
            log_prob_ham += log_prob_word_ham
